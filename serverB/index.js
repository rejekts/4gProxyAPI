...
app.listen(PORT, () => console.log(`ServerB Listening on port ${PORT}`));
                    ...
                    .then(resetClientNewIP => {
                      ...
                      proxyServer
                        ...
                        .then(successfulResetUpdateRez => {
                          ...
                          res.status(200).send(successfulResetUpdateRez.attrs);
                        })
                        .catch(err => {
                          if (err) {
                            console.log(
...
/*
//interval func attempt at running fetch until it succeeded
let timesCalled = 0;
                          setInterval(() => {
                            timesCalled++;
                            if (timesCalled <= 20) {
                              fetch(
                                `http://localhost:10080/api/proxy/browserIP?proxyServerID=${proxyServerID}&status=REBOOTING`,
                                { method: 'GET' }
                              )
                                .then(data => {
                                  console.log(
